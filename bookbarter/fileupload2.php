...
{
	$check=getimagesize($_FILES["myfile2"]["tmp_name"]);
	if ($check!==false) 
	{
	...
	if ($_FILES["myfile2"]["size"] > 100000000) 
	{
	...
	else
	{
		...
		if (move_uploaded_file($_FILES["myfile2"]["tmp_name"], $target_file)) 
		{
			echo "File".basename($_FILES['myfile2']['name']) ." has been successfully uploaded.";
		}
		else
		{
